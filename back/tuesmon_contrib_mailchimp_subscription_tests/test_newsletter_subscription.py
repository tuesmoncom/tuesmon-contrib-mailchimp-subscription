...
from tuesmon.users.signals import user_cancel_account as user_cancel_account_signal
...
def test_user_registry_signal_ko(settings, client):
    user = f.UserFactory.create()
    with mock.patch.object(mailchimp.Lists, 'subscribe') as subscribe_mock_method, \
            mock.patch('tuesmon_contrib_mailchimp_subscription.utils.logger') as mock_logger:
